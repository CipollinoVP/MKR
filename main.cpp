...
int main() {
    ...
    M[0][m-1] = -E*a*tau/(2*h*a*tau-2*h*a*nu*nu*tau);
    M[0][2*m-1] = E*h*nu*tau/(h*a*tau-h*a*nu*nu*tau);
    M[0][3*m-1] = E*a*tau/(2*h*a*tau-2*h*a*nu*nu*tau);
    M[0][n*m+m] = -E*h*nu/(2*h*a*tau-2*h*a*nu*nu*tau);
    M[0][n*m+2*m+1] = E*h*nu/(2*h*a*tau-2*h*a*nu*nu*tau);
    M[1][m-1] = -E*a*tau*nu/(2*h*a*tau-2*h*a*nu*nu*tau);
    M[1][2*m-1] = E*h*tau/(h*a*tau-h*a*nu*nu*tau);
    M[1][3*m-1] = E*a*nu*tau/(2*h*a*tau-2*h*a*nu*nu*tau);
    M[1][n*m+m] = -E*h/(2*h*a*tau-2*h*a*nu*nu*tau);
    M[1][n*m+2*m+1] = E*h/(2*h*a*tau-2*h*a*nu*nu*tau);
    for (int i = 1; i < n-1; ++i) {
        for (int j = 1; j < m-1; ++j) {
    ...
    for (int j = jn+1; 2*M_PI*j/m < 8*M_PI/4; ++j) {
        M[2*m*(n-1)+2*j][(n-1)*m+j] = 1;
        B[2*m*(n-1)+2*j] = -U*std::cos(2*M_PI*j/m);
        M[2*m*(n-1)+2*j+1][n*m+(n-1)*m+j] = 1;
        B[2*m*(n-1)+2*j+1] = U*std::sin(2*M_PI*j/m);
